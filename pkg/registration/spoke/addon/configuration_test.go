...
func TestGetRegistrationConfigs(t *testing.T) {
	...
	}{
		{
			...
			addon: &addonapiv1beta1.ManagedClusterAddOn{
				ObjectMeta: metav1.ObjectMeta{
					...
					Name:      addOnName,
				Annotations: map[string]string{addonapiv1beta1.InstallNamespaceAnnotation: addOnNamespace},
			},
			...
			Spec: addonapiv1beta1.ManagedClusterAddOnSpec{},
				Status: addonapiv1beta1.ManagedClusterAddOnStatus{
					Registrations: []addonapiv1beta1.RegistrationConfig{
						{
						Type: addonapiv1beta1.RegistrationType("kubeClient"),
			...
			addon: &addonapiv1beta1.ManagedClusterAddOn{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						addonapiv1beta1.HostingClusterNameAnnotationKey: "test",
					},
				},
				Spec: addonapiv1beta1.ManagedClusterAddOnSpec{				},
				Status: addonapiv1beta1.ManagedClusterAddOnStatus{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			configs, err := getRegistrationConfigs(c.addon.Name, installOption, c.addon.Status.Registrations, getKubeClientDriver(c.addon.Status.Registrations))
			if err != nil {
